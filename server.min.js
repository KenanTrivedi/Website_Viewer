...
const userDataSchema = new mongoose.Schema({
  ...
  preSurveyResponses: { type: Object, default: {} },
  currentSection: { type: Number, default: -2 },
...
app.post("/api/reset-user-data", async (req, res) => {
  ...
  try {
    ...
    // Preserve pre-survey responses and T1 personal info
    const preservedData = {
      preSurveyResponses: { ...user.preSurveyResponses },
      initialResponses: {
        q0_0: user.initialResponses.q0_0,
        q0_1: user.initialResponses.q0_1,
        q0_2: user.initialResponses.q0_2,
        q0_3: user.initialResponses.q0_3,
        q0_4: user.initialResponses.q0_4,
        q0_5: user.initialResponses.q0_5,
        q0_6: user.initialResponses.q0_6,
      },
    };
    user.initialResponses = preservedData.initialResponses;
    user.updatedResponses = {};
    user.followUpResponses = {};
    user.updatedScores = {};
    user.followUpScores = {};
    ...
    user.t2Complete = false;
    user.t3Complete = false;
    user.preSurveyResponses = preservedData.preSurveyResponses; // Preserve pre-survey data
    await user.save();
...
app.post("/api/save-user-data", async (req, res) => {
  try {
    ...
    if (!user) {
      user = new UserData({
        userId,
        currentSection: currentSection !== undefined ? currentSection : -2,
    ...
    if (Array.isArray(courses)) {
      ...
      user.courses = courses.split(",").map((c) => c.trim());
    } else {
      user.courses = [];
    }
    ...
    if (attemptNumber === 2 && user.openEndedResponses.t2_course_list) {
      const courseString = user.openEndedResponses.t2_course_list;
      user.courses =
        typeof courseString === "string" && courseString.trim() !== ""
          ? courseString.split(",").map((c) => c.trim())
          : [];
    }

    if (currentSection === -2 && preSurveyResponses) {
      if (preSurveyResponses["q-2_0"] === "Nein") {
        preSurveyResponses["q-2_1"] = "Gruppe A";
      }
    ...
    if (isComplete) {
      ...
      if (currentSection === -2) {
        user.preSurveyResponses = { ...user.preSurveyResponses, ...data };
        if (user.preSurveyResponses["q-2_0"] === "Nein") {
    ...
    const responseData = {
      ...
      preSurveyResponses: user.preSurveyResponses, // Added to return full preSurveyResponses
    };
...
app.get("/api/user-data/:userId", async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({
      ...
      preSurveyResponses: user.preSurveyResponses,
      currentSection: user.currentSection || -2,
...
app.get("/api/dashboard-data", authenticate, async (req, res) => {
  try {
    ...
    for (const doc of allUsers) {
      ...
      // Start with initialResponses as the base
      let personalData = { ...doc.initialResponses };

      // Layer updatedResponses on top of initialResponses
      personalData = { ...personalData, ...doc.updatedResponses };

      // Layer followUpResponses on top of updatedResponses and initialResponses
      personalData = { ...personalData, ...doc.followUpResponses };
      ...
      users.push({
        ...
        preSurveyResponses: doc.preSurveyResponses || {},
...
app.post("/api/save-open-ended-response", async (req, res) => {
  ...
  // Define valid open-ended response keys
  const validKeys = [
    "t1_reflection",
    "t2_reflection",
    "t2_course_feedback",
  ...
  try {
    const updatedUser = await UserData.findOneAndUpdate(
      { userId: userId },
      { $set: { [`openEndedResponses.${key}`]: response } },
