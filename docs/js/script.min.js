...
function createCompetencyChartConfig(
  ...
  const config = {
    ...
    options: {
      ...
      // Moved onHover directly under options
      onHover: (event, activeElements, chart) => {
        ...
        if (activeElements && activeElements.length > 0 && descriptionBox) {
          ...
          console.log('No active elements or description box not found')
        }
      },
    },
  }
...
async function handleLogin() {
  ...
  try {
    ...
    const payload = {
      code: loginCodeValue,
      courses: t2CourseText,
    }
    
    // Set the correct parameters based on the survey option
...
function setupStartSurveyButton() {
  ...
  if (startSurveyButton) {
    startSurveyButton.addEventListener('click', async function () {
      try {
        // Get the userId and generatedCode from sessionStorage
        ...
        if (!userId || !generatedCode) {
          Swal.fire({
            icon: 'error',
            title: 'Fehler',
            text: 'Benutzer-ID oder Code nicht gefunden. Bitte erneut registrieren.',
          })
          window.location.href = 'generateCode.html'
          return
        }

        // Set the attemptNumber to 1 for new users
        sessionStorage.setItem('attemptNumber', '1')

        // Initialize the user session by making a login request
        const payload = {
          code: generatedCode,
          surveyOption: 'register',
          t2CourseList: '',
        }
        ...
        // Then navigate to the survey page
        window.location.href = 'survey.html'
      } catch (err) {
