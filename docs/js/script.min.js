...
function createCompetencyChartConfig(
  ...
  t3Data,
  canvasId,
  descriptionBoxId, // RE-INTRODUCE THIS ARGUMENT
  userCode
) {
  const datasets = []
  ...
  const config = {
    ...
    options: {
      ...
      plugins: {
        ...
        annotation: {
          // This assumes you might want to use a Chart.js annotation plugin later, for simple text, we can use 'title' plugin as well or just drawText on canvas directly if needed.
          annotations: [
            {
              type: 'label',
              position: 'top-right', // Position of the user code text
              xValue: 'end', // Align to the end of the x-axis
              yValue: 100, // Position near the top (adjust as needed)
              xAdjust: -10, // Adjust horizontal position
              yAdjust: -10, // Adjust vertical position
              content: `User Code: ${userCode}`, // Text to display
              font: {
                size: 12,
                style: 'italic',
                color: 'grey',
        ...
        title: {
          // Alternative using Chart.js 'title' plugin
          display: true,
          position: 'bottom',
          align: 'end',
        ...
        // --- END OF ANNOTATION PLUGIN CONFIGURATION ---
        onHover: (evt, item) => {
          // RE-INTRODUCE ONHOVER PLUGIN
          const descriptionBox = document.getElementById(descriptionBoxId) // RE-INTRODUCE DESCRIPTION BOX ID
          if (item.length > 0 && descriptionBox) {
            // RE-INTRODUCE && descriptionBox
            ...
            ) // RE-INTRODUCE UPDATE DESCRIPTION BOX
          }
        },
      },
    },
