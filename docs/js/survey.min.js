...
function lockPersonalFields() {
  ...
  form.querySelectorAll('input, select, textarea').forEach((el) => {
    // Skip locking t2_course_feedback if this is T2
    if (el.name === 't2_course_feedback') {
      if (attemptNumber === 2) {
...
function nextSection() {
  if (currentSection === -1) {
    const form = document.getElementById('surveyForm')
    let valid = validateSection(true) //true is not used
    if (!form.checkValidity()) {
      valid = false
    }
    if (valid && validateDatenschutz()) {
      // Save the Datenschutzerklärung (data protection) info.
      ...
      // Save partial data with isComplete = false.
      saveSectionData(false, {
        // No need to await here, we move to the next section regardless
        datenschutzConsent: userData.datenschutzConsent,
        unterschrift: userData.unterschrift,
      })
        .then(() => {
          // Use .then() to remove the token *after* saving
          localStorage.removeItem('surveyResumeToken')
          currentSection = 0
        ...
        .catch((error) => {
          //saveSectionData already handles displaying errors,
          //but we need this catch block so the promise chain doesn't break
          console.error('Error in nextSection after saving Datenschutz:', error)
        })
    } else {
    ...
    if (validateSection()) {
      // Save partial progress (again with isComplete = false).
      saveSectionData(false)
        .then(() => {
          currentSection++
          renderSection(currentSection)
        ...
        .catch((error) => {
          //saveSectionData already handles displaying errors
          console.error(
            'Error in nextSection after saving section data:',
            error
          )
        })
    } else {
    ...
    // Last section: finish the survey and mark it complete.
    finishSurvey()
  }
...
function previousSection() {
  if (currentSection === 0) {
    ...
    // Add a check for unsaved changes before navigating back
    if (hasUnsavedChanges()) {
      ...
      return // Prevent further execution if the user cancels
    }
    currentSection--
  }
  renderSection(currentSection)
...
function updateNavigationButtons() {
  ...
  const isT1Initial = attemptNumber === 1 && currentSection === -1

  if (isT1Initial) {
    // Datenschutz page: show only one "Weiter" button.
    navDiv.innerHTML = `
    ...
    // All other survey pages: show Back, Save Progress, and Next buttons.
    navDiv.innerHTML = `
...
async function loadUserData() {
  try {
    ...
    // Initialize userData based on whether data was found
    if (data && Object.keys(data).length > 0) {
      ...
      userData = {
        ...
        currentSection:
          data.currentSection !== undefined
            ...
            : attemptNumber === 1
            ? -1
            : 0, // Use server's currentSection, default based on attempt
        t1Complete: data.t1Complete || false, // Use server's completion flags
        t2Complete: data.t2Complete || false,
        t3Complete: data.t3Complete || false,
        initialResponses: data.initialResponses || {}, // Add this line
        updatedResponses: data.updatedResponses || {}, // Add this line
        followUpResponses: data.followUpResponses || {}, // Add this line
      }
    } else {
      ...
      userData = {
        ...
        scores: { t1: {}, t2: {}, t3: {} },
        openEndedResponses: {},
        attemptNumber: 1, // Always 1 for a brand new user
        datenschutzConsent: false,
        unterschrift: '',
        timeStamps: { t1: null, t2: null, t3: null },
        courses: [],
        isComplete: false,
        currentSection: -1, // Start at Datenschutz for new T1
        t1Complete: false,
        t2Complete: false,
        t3Complete: false,
        initialResponses: {}, // Add this line
        updatedResponses: {}, // Add this line
        followUpResponses: {}, // Add this line
      }
    }
    ...
    isInitializing = false // Ensure this is always reset
  }
}
...
async function saveSectionData(isComplete, additionalData = {}) {
  ...
  if (!userId) {
    //This should never happen, but it's good to have a check
    Swal.fire({
      icon: 'error',
      title: 'Error',
      text: 'User ID not found. Please log in again.',
  ...
  if (attemptNumber === 2) {
    if (payload.openEndedResponses.t2_course_list) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(errData.message || 'Server error') // More specific error
    }
    ...
    if (data.initialScores) {
      userData.scores.t1 = data.initialScores
    }
    if (data.updatedScores) {
      userData.scores.t2 = data.updatedScores
    }
    if (data.followUpScores) {
      userData.scores.t3 = data.followUpScores
    }

    if (data.isComplete) {
      sessionStorage.setItem('surveyCompleted', 'true')
      // Remove the resume token *only* if the save is complete and successful
      localStorage.removeItem('surveyResumeToken')
    }
  ...
  fetch('/api/save-open-ended-response', {
    ...
    .then(() => {
      ...
      Swal.fire({
        ...
        icon: 'success',
        title: 'Gespeichert',
        text: 'Ihre Antwort wurde gespeichert!',
        timer: 2000,
        showConfirmButton: false,
      })
      showCourseLinks()
...
function validateSection(checkRequired = false) {
  ...
  form.querySelectorAll('[required]').forEach((field) => {
    ...
    // Check if this field is within a conditional field related to q0_2
    const conditionalParent = field.closest('.conditional-field')
    if (conditionalParent) {
      const controllingRadioName = 'q0_2' // The name of the controlling radio button
      const controllingRadio = form.querySelector(
        `input[name="${controllingRadioName}"]:checked`
      )

      if (controllingRadio) {
        const condition = conditionalParent.dataset.condition // 'Ja' or 'Nein'
        if (controllingRadio.value !== condition) {
          // If the controlling radio's value doesn't match the condition, skip validation
          return
        }
      } else {
        // IMPORTANT CHANGE: If no controlling radio is checked, *don't* skip validation.
        // This ensures that the required fields are validated even on initial load.
        // Previously, we were skipping validation if no radio was checked, which was incorrect.
      }
    }
  ...
  if (!valid) {
    // Only show the general message if validation fails
    Swal.fire({
      icon: 'error',
      title: 'Fehler',
...
function renderSection(sectionIndex) {
  ...
  // If we're at sectionIndex -1, show Datenschutz
  if (sectionIndex === -1) {
    surveyForm.innerHTML = `
        ...
        <div class="datenschutz-inputs">
          ...
          <div class="question">
            ...
            <input type="text" id="unterschrift" name="unterschrift"
                   value="${sessionStorage.getItem('generatedCode') || ''}"
                   readonly required>
          </div>
  ...
  // If the sectionIndex is invalid, reset to -1 and re-render
  ...
  section.questions.forEach((question, qIndex) => {
    ...
    if (question.type === 'radio') {
      // Get responses and fieldValue *before* the options loop
      const responses = getResponseObject(userData, attemptNumber)
      const fieldValue =
        sectionIndex === 0
      ...
      question.options.forEach((opt) => {
        const isChecked = fieldValue === opt // Correct comparison
      ...
      // Conditional Logic for q0_2 (Studieren Sie Lehramt?)
      if (fieldName === 'q0_2') {
        ...
        <div class="conditional-fields-container">
          ...
          !isLocked && attemptNumber === 1 ? 'required' : ''
            ...
            <div class="question">
              ...
              <input type="text" name="q0_7" ${isLocked ? 'readonly' : ''} ${
          !isLocked && attemptNumber === 1 ? 'required' : ''
        } value="${getPersonalDataValue(userData, attemptNumber, 'q0_7')}"/>
            </div>
          </div>
          ...
          <div class="conditional-field" data-condition="Nein" style="display: ${
            ...
            <div class="question">
              ...
              <input type="text" name="q0_8" ${isLocked ? 'readonly' : ''} ${
          !isLocked && attemptNumber === 1 ? 'required' : ''
        }  value="${getPersonalDataValue(userData, attemptNumber, 'q0_8')}"/>
            </div>
          </div>
        </div>
        `
  ...
  // Add T2 Feedback section if attemptNumber is 2 or 3
  if (sectionIndex === 0 && (attemptNumber === 2 || attemptNumber === 3)) {
    let feedback = ''
    let feedbackField = ''

    if (attemptNumber === 2) {
      feedback = userData.openEndedResponses?.t2_course_feedback || ''
      feedbackField = `
                <div class="t2-feedback">
                    <p>Wie zufrieden sind Sie mit den angebotenen Kursen zur Förderung digitaler Kompetenzen?</p>
                    <textarea name="t2_course_feedback" required>${feedback}</textarea>
                </div>`
    } else if (attemptNumber === 3) {
      feedback = userData.openEndedResponses?.t2_course_feedback || ''
      feedbackField = `
                <div class="t2-feedback readonly">
                    <p>Wie zufrieden sind Sie mit den angebotenen Kursen zur Förderung digitaler Kompetenzen? (Ihre vorherige Antwort):</p>
  ...
  if (radios.length > 0) {
    radios.forEach((radio) => {
      radio.addEventListener('change', () => {
        handleTeachingStudentChange(radio)
      })
    })

    // Find the currently checked radio button, if any
    let checkedRadio = document.querySelector('input[name="q0_2"]:checked')

    // Call handleTeachingStudentChange:
    // - If a radio is checked, use it to set the initial state.
    // - If no radio is checked (initial load AND no previous selection),
    //   we DON'T call it here.  We'll call it AFTER setting initial values.
    if (checkedRadio) {
      handleTeachingStudentChange(checkedRadio)
    }
  }

  // Set initial values for conditional fields, then call handleTeachingStudentChange
  const q0_2Value = getPersonalDataValue(userData, attemptNumber, 'q0_2')
  if (q0_2Value) {
    // Only if q0_2 has a value
    const checkedRadio = document.querySelector(
      `input[name="q0_2"][value="${q0_2Value}"]`
    )
    if (checkedRadio) {
      handleTeachingStudentChange(checkedRadio) // NOW we call it, AFTER setting values
    }
  }
  ...
  // Add event listeners for scale rating and conditional logic
...
function handleTeachingStudentChange(radio) {
  ...
  // Find the container for conditional fields
...
function validateNumberInput(input) {
  ...
  let cleanedValue = input.value.replace(/[^0-9]/g, '') // REMOVED hyphen

  // Enforce min and max (important for validation, not just visual)
  let numValue = parseInt(cleanedValue, 10)
  if (isNaN(numValue)) {
    // If it's empty or otherwise not a number, don't enforce min/max yet
  ...
  input.value = numValue // Update the input value
}
...
async function showResults() {
  const userId = sessionStorage.getItem('userId')
  if (!userId) {
    //sessionStorage.setItem('surveyCompleted', 'true'); //This line seems misplaced, it should be set after saving, not before fetching //REMOVED
    Swal.fire({
      icon: 'error',
      title: 'Fehler',
  ...
  try {
    ...
    // *** CRITICAL CHANGE: Merge fetched data with existing userData ***
    userData = {
      ...userData, // Keep existing data
      ...data, // Overwrite with the latest data from the server
      responses: {
        ...(userData.responses || {}),
        ...data.responses,
      },
      ...
      openEndedResponses: {
        ...(userData.openEndedResponses || {}), // Keep existing open-ended responses!
        ...data.openEndedResponses,
      },
    }
    ...
    // Determine which scores to use for the overall score
    const finalScores =
    ...
    // Get the overall score from the correct set of scores
    const overallScore = finalScores.overall || 0

    let html = `
      <h2>Ihr Kompetenzscore beträgt ${overallScore}%</h2>
      ...
      <p>Das folgende Diagramm zeigt Ihre Scores in verschiedenen Kompetenzbereichen.${
        Object.keys(followUpScores).length > 0
          ? ' Die helleren Balken repräsentieren Ihre Ergebnisse nach der ersten Befragung (T1), die mittleren Balken repräsentieren Ihre Ergebnisse nach der zweiten Befragung (T2) und die dunkleren Balken Ihre Ergebnisse nach der aktuellen Befragung (T3).'
          : Object.keys(updatedScores).length > 0
    ...
    if (attemptNumber === 1) {
      ...
      let t2Reflection = userData.openEndedResponses?.t2_reflection || '' // Get T2 reflection
      html += `
      ...
      let t3Reflection = userData.openEndedResponses?.t3_reflection || '' // Get T3 reflection
      html += `
