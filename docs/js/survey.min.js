  ...
  try {
    ...
    if (group === 'Gruppe D') {
      ...
      `
        ...
        // Modified T3 implementation with two steps
        const isFirstStep = !sessionStorage.getItem('t3FirstStepCompleted')

        if (isFirstStep) {
          // Step 1: Show initial reflection question
          document.getElementById('surveyForm').innerHTML = `
          ...
          document
            ...
            .addEventListener('click', async (e) => {
              ...
              const response = document
                .getElementById('t3OpenEndedResponse')
                .value.trim()
              ...
              // Save first response
              await fetch('/api/save-open-ended-response', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                  userId: sessionStorage.getItem('userId'),
              ...
              sessionStorage.setItem('t3FirstStepCompleted', 'true')
              ...
              document.getElementById('surveyForm').innerHTML = `
                  ...
                  <p style="margin-top: 20px;">Im Folgenden sehen Sie nun Ihre Entwicklung im Verlauf des Projektes und Ihre durchschnittlichen Einschätzungen für die einzelnen Kompetenzbereiche. Abschließend würden wir gerne wissen, was Sie denken. Passen Ihre Einschätzungen zu den Ergebnissen aus der Grafik?</p>
          ...
          // If user refreshes page after completing first step, show second step
          document.getElementById('surveyForm').innerHTML = `
              <div style="height: 300px; width: 100%;">
                <canvas id="competencyChart1"></canvas>
              </div>
  ...
  ${
    group !== 'Gruppe A'
      ? `<p style="text-align: center; margin-top: 10px; margin-bottom: 30px;">
          Sie können die Website nun schließen oder zur <a href="login.html">Startseite</a> zurückkehren.
         </p>`
      : ''
  }
  <hr>
...
`
    ...
    // Check if progressBar and progressText exist before accessing them
    ...
    if (group !== 'Gruppe B' && group !== 'Gruppe C') {
      ...
      // For Gruppe C, create chart but don't add event listener in T1
      createCompetencyChart1(initialScores, updatedScores, followUpScores)
      document
        .getElementById('downloadChart')
        .addEventListener('click', downloadChart)

      // Add confetti animation for Group C when they see the results page
      if (group === 'Gruppe C') {
        setTimeout(() => {
          confetti({
            particleCount: 100,
    ...
    if (attemptNumber === 1) {
      ...
      const submitT1Button = document.getElementById(
        'submitT1OpenEndedResponse'
      )
      if (submitT1Button) {
        submitT1Button.addEventListener('click', submitT1OpenEndedResponse)
      }
    } else if (attemptNumber === 2) {
      // Only add event listener if the element exists
      const submitT2Button = document.getElementById(
        'submitT2OpenEndedResponse'
      )
      if (submitT2Button) {
        submitT2Button.addEventListener('click', submitT2OpenEndedResponse)
      }
    } else if (attemptNumber === 3) {
      // Only add event listener if the element exists
      const submitT3Button = document.getElementById(
        'submitT3OpenEndedResponse'
      )
      if (submitT3Button) {
        submitT3Button.addEventListener('click', submitT3OpenEndedResponse)
      }
    }
    hideNavigationButtons()
  } catch (error) {
    ...
    .then(() => {
      ...
      if (group === 'Gruppe D') {
        ...
        surveyForm.innerHTML = `
                    ...
                    <div id="descriptionBox1"></div>
                    <p style="margin-top: 20px;">Im Folgenden sehen Sie nun Ihre Entwicklung im Verlauf des Projektes und Ihre durchschnittlichen Antworten für die einzelnen Kompetenzbereiche. Abschließend würden wir gerne wissen, was Sie denken. Passen Ihre Einschätzungen zu den Ergebnissen aus der Grafik?</p>
                    <textarea id="t3AdditionalResponse" rows="4" style="width:100%;" required></textarea>
                    <button id="submitT3AdditionalResponse" class="btn btn-primary">Absenden</button>
                `
        createCompetencyChart1(
        ...
        // Add course links for Group C in T3
        if (group === 'Gruppe C') {
          message =
            'Danke für Ihre Teilnahme. Wenn Sie Lust haben, sich in bestimmten Kompetenzbereichen weiterzuentwickeln, dann absolvieren Sie gerne unsere Mikrofortbildungen.<br><br><div class="ilias-links"><ul><li><a href="https://ilias.uni-rostock.de/goto.php?target=crs_121177&client_id=ilias_hro" target="_blank">Suchen, Verarbeiten und Aufbewahren</a></li><li><a href="https://ilias.uni-rostock.de/goto.php?target=crs_122050&client_id=ilias_hro" target="_blank">Analysieren und Reflektieren</a></li><li><a href="https://ilias.uni-rostock.de/goto.php?target=crs_120680&client_id=ilias_hro" target="_blank">Kommunikation und Kollaborieren</a></li><li><a href="https://ilias.uni-rostock.de/goto.php?target=crs_121178&client_id=ilias_hro" target="_blank">Produzieren und Präsentieren</a></li><li><a href="https://ilias.uni-rostock.de/goto.php?target=crs_121179&client_id=ilias_hro" target="_blank">Schützen und sicher Agieren</a></li><li><a href="https://ilias.uni-rostock.de/goto.php?target=crs_121180&client_id=ilias_hro" target="_blank">Problemlösen und Handeln</a></li></ul></div>'
        } else {
          message =
            'Danke für Ihre Teilnahme. Sie können die Website nun schließen oder zur <a href="login.html">Startseite</a> zurückkehren.'
        }
