...
function renderSection(sectionIndex) {
  ...
  if (sectionIndex === -2 && attemptNumber === 1) {
    ...
    preSurveyQuestions.questions.forEach((question, qIndex) => {
      ...
      if (question.type === 'radio') {
        ...
        question.options.forEach((opt) => {
          // Only attach onchange to participation question (q-2_0)
          const onchangeAttr =
            fieldName === 'q-2_0'
    ...
    const currentFormData = getFormData() // Get current form state
    ...
    // Also check for current selections in the form
    ...
    // Trigger conditional logic on load, but only if no radio is checked yet
    ...
    // Render main sections from survey-data.min.js for all attempts
    const section = surveyData[sectionIndex]
    ...
    // Add event listeners for scale ratings
    ...
    currentSection = attemptNumber === 1 ? -2 : 0 // Reset to start based on attempt
    renderSection(currentSection)
    return
  }
